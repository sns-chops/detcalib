...
difc2 = np.ma.masked_array(difc2, mask)
...
#L1 = 20.0965
L1 = 20.0114
sin_theta = difc2/(0.0015882549421289758*1e6) * np.pi / (L1+L2)
...
for pack in packs:
    ...
    # fit = align.FitPack_DifcL2(pack_model, options, difc2, L2, logger)
    # fit = align.FitPackDifc(pack_model, options, difc, logger)
    fit = align.FitPackDifc(pack_model, options, difc2, logger)
    try:
        fit.fit()
